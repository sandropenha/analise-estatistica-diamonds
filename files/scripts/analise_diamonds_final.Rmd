...
2. É muito improvavel que exista diamantes com ~30cm e ~60cm, mas ainda que existam, eles custariam milhares de dolares, o que podemos ver que não acontece.
...
É possível ver também que há correlação negativa moderada entre qualidade e tabela (largura do topo do diamante em relação ao ponto mais largo), ou seja, quanto menor a largura do topo em relação ao ponto mais largo do diamante, sua qualidade tende a aumentar.
...
Após realização da análise preditiva, realizaremos aqui a construção de um modelo de machine learning que possa prever com precisão o preço que será aplicado a venda dos diamantes.

Já vimos anteriormente na análise preditiva que, o principal fator que compõe o preço final de um diamante é o seu peso em kilogramas.

Vimos também, que a qualidade do diamante não é determinante ao seu valor final.

#### **Verificando variáveis multicolineares**
```{r}
num_vars <- sapply(df4,is.numeric)
...
Comprimento e largura possuem multicolinearidade, teremos que dropar essas variáveis ou o modelo poderá ficar instável e tendencioso.

```{r}
modelo1 <- lm(preco~.,df);summary(modelo1)
```
A primeira vista, nosso modelo ficou muito bom, possui variáveis significantes, R2 alto, p-value baixo. Entretanto ainda há variáveis que possuem multicolinearidade, além dos valores numéricos estarem em escalas diferentes. Vamos realizar esses ajustes e em seguida separar os dados entre dados de treino e teste para criação de um novo modelo preditivo mais assertivo.


#### **Normatização e drop de variáveis multicolineares**
```{r}
normatizados <- df4 %>% select(-preco) %>% mutate_if(is.numeric,scale);head(normatizados)
target <- df4 %>% select(preco);head(target)
dados_normatizados <- cbind(normatizados,target);head(dados_normatizados)
```

```{r}
df <- dados_normatizados %>% select(-comprimento,-largura)
```


```{r}
treino <- df %>% sample_frac(0.7)
teste <- df %>% sample_frac(0.3)
```

#### **Modelos otimizados **

#### **Imprimindo variáveis mais importantes**
```{r}
control <- trainControl(method = "repeatedcv", number = 10, repeats = 2 )
model <- train(preco~., data = treino, method = "glm")
importance <- varImp(model,scale = FALSE)
plot(importance)
```

Vemos aqui que, de longe, a variável mais importante para a construção do modelo preditivo é a variável peso, podemos cogitar dropar algumas variáveis para que o modelo fique assim mais genérico e estável.


```{r}
modelo2 <- lm(preco~peso+claridade+cor,data=treino);summary(modelo2)
modelo3 <- lm(preco~peso,treino);summary(modelo3)
```

Dropar as variáveis praticamente não afetou o desempenho do R2 do modelo, além de deixa-lo mais estável e menos enviesado. Em outras palavras, o modelo 2 é o que apresenta melhor desempenho. mas vamos confirmar esse hipotese analisando mais a fundo os três modelos criados.

####**Analise residual**##

```{r}
par(mfrow=c(1,2))
hist(modelo2$residuals, labels = TRUE, ylim = c(0,30000), main = "Modelo2 - variaveis ajustadas") 
hist(modelo3$residuals, labels = TRUE, ylim = c(0,30000), main = "Modelo3 - somente variavel peso") 
```

Nesse plot, vemos que o desempenho residual do modelo 2 foi melhor que a do modelo 3, pois a concentração dos residuals está mais localizado ao centro (zero)
...
Também é possível aqui que o modelo 2 também apresenta menos fuga dos residuals em torno do zero

#### **Ajustando dados de teste para o formato dos de treino:**

```{r}
teste <- teste %>% select(preco,peso,claridade,cor)
```

####**Fazendo previsoes:**
```{r}
previsao <- predict(modelo3,teste)
mean(previsao)
mean(teste$preco)
```


Comparando também a média dos valores previstos com os valores do dataset de testes vemos que ficaram muito próximas. O que é muito bom.

```{r}
previsao <- as.data.frame(previsao)
df_previsto <- cbind(teste,previsao)
head(df_previsto)
```
...
df_previsto <- df_previsto %>% 
  select(peso,claridade,cor,preco,previsao) %>% 
  mutate(previsao = round(previsao),
         previsaoxreal = previsao-preco)
```
